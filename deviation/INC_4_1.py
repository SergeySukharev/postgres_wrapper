...
query_4_1_4 = ("""select 	hcode_id, hcode_name, hcode_unit_name, org_id, dor_kod, date_type_id, metric_type_id, cargo_type_id, val_type_id,
		unit_id, dt, value, ss, duch_id, nod_id, dir_id, kato_id, vids_id from dm_rep.dm_all_indicators_v
where 	calc_rule = 'deviation'
	and metric_type_id = 120
	and date_type_id = 3 
	and dt in (""" + date3 + """)
...
select 'calc', val_type_id, count(0) from dm_rep.dm_all_indicators_v
where 	calc_rule = 'deviation'
	and metric_type_id = 22 -- Отклонение факта к плану, %
...
select 	fct.hcode_id, hcd.name, hcd.unit_name, fct.org_id, fct.dor_kod, fct.date_type_id, 22 as metric_type_id, 
		...
		/abs(case when pln.value = 0 then null else pln.value end)*100, fct.ss, fct.duch_id, fct.nod_id, fct.dir_id, fct.kato_id, fct.vids_id
...
query_4_1_7 = ("""select * from (
	select 'calc', val_type_id, count(0) from dm_rep.dm_all_indicators_v
	where 	calc_rule = 'deviation'
		and metric_type_id = 23 -- Отклонение факта к директивному плану, %
		and date_type_id = 3 
		and dt in (""" + date3 + """)
	group by val_type_id
	union all
...
select 	fct.hcode_id, hcd.name, hcd.unit_name, fct.org_id, fct.dor_kod, fct.date_type_id, 23 as metric_type_id, 
		...
		then null else pln.value end)*100, fct.ss, fct.duch_id, fct.nod_id, fct.dir_id, fct.kato_id, fct.vids_id
		from (select * from dm_rep.dm_all_indicators_v where metric_type_id = 17) fct
        join dm_rep.d_hcode_v hcd on fct.hcode_id = hcd.id
		left join (select * from dm_rep.dm_all_indicators_v where metric_type_id = 2) pln
			on (	fct.hcode_id = pln.hcode_id
...
select 	cd.hcode_id, hcd.name, hcd.unit_name, cd.org_id, cd.dor_kod, cd.date_type_id, 28 as metric_type_id, cd.cargo_type_id, 
		cd.val_type_id, cd.unit_id, cd.dt, (cd.value - pyd.value), cd.ss, cd.duch_id, cd.nod_id, cd.dir_id, cd.kato_id, cd.vids_id
...
select 	fct.hcode_id, hcd.name, hcd.unit_name, fct.org_id, fct.dor_kod, fct.date_type_id, 25 as metric_type_id, fct.cargo_type_id, 
		fct.val_type_id, fct.unit_id, fct.dt, (fct.value - pln.value), fct.ss, fct.duch_id, fct.nod_id, fct.dir_id, fct.kato_id, fct.vids_id
		from (select * from dm_rep.dm_all_indicators_v where metric_type_id = 17) fct
        join dm_rep.d_hcode_v hcd on fct.hcode_id = hcd.id
		left join (select * from dm_rep.dm_all_indicators_v where metric_type_id = 12) pln
			on (	fct.hcode_id = pln.hcode_id
...
query_4_1_17 = ("""select * from (
	...
	group by val_type_id
	union all
	select 'join', drs.val_type_id, count(0) from
	(select * from dm_rep.dm_all_indicators_v
		where 	val_type_id in (1,3,7,8)	
			...
			and hcode_id in ('00049', '00053')
			and dor_kod = 41) st
		join (select * from dm_rep.dm_all_indicators_v where dor_kod <> 41) drs
			on (	st.hcode_id = drs.hcode_id
				and st.date_type_id = drs.date_type_id
				and st.metric_type_id = drs.metric_type_id
				and st.cargo_type_id = drs.cargo_type_id
				and st.val_type_id = drs.val_type_id
				and st.unit_id = drs.unit_id
				and st.dir_id = drs.dir_id
				and st.duch_id = drs.duch_id
				and st.nod_id = drs.nod_id
				and st.kato_id = drs.kato_id
				and st.vids_id = drs.vids_id
				and st.dt = drs.dt)
		where st.dt in (""" + date3 + """)
	group by drs.val_type_id
)a 
order by a.val_type_id""")
...
select 	dor.hcode_id, hcd.name, hcd.unit_name, dor.org_id, dor.dor_kod, dor.date_type_id, '112' as metric_type_id, 
		dor.cargo_type_id, dor.val_type_id, dor.unit_id, dor.dt, (dor.value - st.value) as value, dor.ss, dor.duch_id, dor.nod_id, dor.dir_id, dor.kato_id, dor.vids_id
from
(select * from dm_rep.dm_all_indicators_v
	where 	val_type_id in (1,3,7,8)	
		and metric_type_id = 17
...
select 	fct.hcode_id, hcd.name, hcd.unit_name, fct.org_id, fct.dor_kod, fct.date_type_id, 42 as metric_type_id, 
		fct.cargo_type_id, fct.val_type_id, fct.unit_id, fct.dt, (fct.value/pln.value)*100 as value, fct.ss, fct.duch_id, fct.nod_id, fct.dir_id, fct.kato_id, fct.vids_id
	from (select * from dm_rep.dm_all_indicators_v where metric_type_id = 17 and date_type_id = 3) fct
    join dm_rep.d_hcode_v hcd on fct.hcode_id = hcd.id
	left join (select * from dm_rep.dm_all_indicators_v where metric_type_id = 12 and date_type_id = 5 and val_type_id = 7) pln
			on (	fct.hcode_id = pln.hcode_id
...
select 	fct.hcode_id, hcd.name, hcd.unit_name, fct.org_id, fct.dor_kod, fct.date_type_id, 26 as metric_type_id, 
		...
		fct.ss, fct.duch_id, fct.nod_id, fct.dir_id, fct.kato_id, fct.vids_id
		from (select * from dm_rep.dm_all_indicators_v where metric_type_id = 17) fct
        join dm_rep.d_hcode_v hcd on fct.hcode_id = hcd.id
		left join (select * from dm_rep.dm_all_indicators_v where metric_type_id = 12) pln
			on (	fct.hcode_id = pln.hcode_id
...
query_4_1_25 = ("""select * from (
	...
	select 'join', st.val_type_id, count(0) from (
		...
		where 	val_type_id in (1,3,7,8)	
			...
			and dir_id <> 8
			and dor_kod <> 41) st
		left join (select * from dm_rep.dm_all_indicators_v where dir_id = 8 and dor_kod <> 41) dor
			on (	st.hcode_id = dor.hcode_id
	...
	where hcd.group_name in ('Выполнение расписания пассажирских поездов', 'Выполнение расписания пригородных поездов', 'Выполнение расписания грузовых поездов')
	and st.dt in (""" + date3 + """)
...
# -- Сравнение записей в таблицах --
# ----------------------------------
query_4_1_26 = ("""select 	hcode_id, hcode_name, hcode_unit_name, org_id, dor_kod, date_type_id, metric_type_id, cargo_type_id, val_type_id, unit_id, 
...
left join (select * from dm_rep.dm_all_indicators_v where dir_id = 8 and dor_kod <> 41) st
			on (	st.hcode_id = dor.hcode_id
				and st.date_type_id = dor.date_type_id
				and st.metric_type_id = dor.metric_type_id
				and st.cargo_type_id = dor.cargo_type_id
...
where 	calc_rule = 'deviation'
	...
	and metric_type_id = 201 -- Отклонение плана к факту прошлого года, %
	and date_type_id = 3
    and dt in (""" + date3 + """) 
group by val_type_id
union all
...
query_4_1_30 = ("""select 	hcode_id, hcode_name, hcode_unit_name, org_id, dor_kod, date_type_id, metric_type_id, cargo_type_id, val_type_id, unit_id, 
		dt, value, ss, duch_id, nod_id, dir_id, kato_id, vids_id 
from dm_rep.dm_all_indicators_v
where 	calc_rule = 'deviation'
	and val_type_id in (1,3,7,8)
	and metric_type_id = 201 -- Отклонение плана к факту прошлого года, %
...
select 	pln.hcode_id, hcd.name, hcd.unit_name, pln.org_id, pln.dor_kod, pln.date_type_id, 201 as metric_type_id, 
		...
		/abs(case when fct.value = 0 then null else fct.value end)*100, pln.ss, pln.duch_id, pln.nod_id, pln.dir_id, pln.kato_id, pln.vids_id
		from (select * from dm_rep.dm_all_indicators_v where metric_type_id = 12) pln
...
query_4_1_33 = ("""select * from (
	select 'calc', val_type_id, count(0) from dm_rep.dm_all_indicators_v
	where 	calc_rule = 'deviation'
		and metric_type_id = 204 -- Отклонение факта к плану с корректировкой, %
		and date_type_id = 3 
		and dt in (""" + date3 + """)
	group by val_type_id
	union all
